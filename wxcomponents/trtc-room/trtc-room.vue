<template>
<uni-shadow-root class="trtc-room-trtc-room"><import src="./template/1v1/1v1.wxml"></import>
  ...
  <view class="im-panel" v-if="enableIM && showIMPanel">
    <view class="message-panel-body">
      <scroll-view scroll-y="true" class="message-scroll-container" :scroll-into-view="'message'+(messageList.length-1)" :scroll-with-animation="true">
        <view class="message-list">
          <view v-for="(item,index) in (messageList)" :key="item.index" class="message-item" :id="'message'+(index)">
    ...
    <view class="message-panel-bottom">
      <view class="message-input-container">
        <input class="message-input" type="text" :value="messageContent" @input="_inputIMMessage" @confirm="_sendIMMessage" confirm-type="send" placeholder="请输入消息" maxlength="200" placeholder-style="color:#ffffff;opacity: 0.55;"></input>
      </view>
      <view class="message-send-btn">
        <button class="btn" @click="_sendIMMessage" hover-class="btn-hover">发送</button>
      </view>
    </view>
    <view @click="_toggleIMPanel" class="close-btn">X</view>
  </view>
  
  <view :class="'debug-info-btn '+(debugMode && !debugPanel?'':'none')">
    <button class="debug-btn" @click="_debugTogglePanel" hover-class="button-hover">Debug</button>
  </view>
  <view :class="'debug-info '+(debugMode && debugPanel?'':'none')">
    <view @click="_debugTogglePanel" class="close-btn">X</view>
    <view>appVersion: {{appVersion}}</view>
    <view>libVersion: {{libVersion}}</view>
    <view>template: {{template}}</view>
    <view>debug: <button :class="(debug?'':'false')+' debug-btn'" @click="_debugToggleVideoDebug" hover-class="button-hover">{{debug}}</button></view>
    <view>userID: {{pusher.userID}}</view>
    <view>roomID: {{pusher.roomID}}</view>
    <view>camera: <button :class="(pusher.enableCamera?'':'false')+' debug-btn'" @click="_toggleVideo" hover-class="button-hover">{{pusher.enableCamera}}</button></view>
    <view>mic: <button :class="(pusher.enableMic?'':'false')+' debug-btn'" @click="_toggleAudio" hover-class="button-hover">{{pusher.enableMic}}</button></view>
    <view>switch camera: <button class="debug-btn" @click="switchCamera" hover-class="button-hover">{{cameraPosition||pusher.frontCamera}}</button></view>
    <view>Room:
      <button class="debug-btn" @click="_debugEnterRoom" hover-class="button-hover">Enter</button>
      <button class="debug-btn" @click="_debugExitRoom" hover-class="button-hover">Exit</button>
      <button class="debug-btn" @click="_debugGoBack" hover-class="button-hover">Go back</button>
    </view>
    <view>IM: <button class="debug-btn" @click="_debugSendRandomMessage" hover-class="button-hover">send</button></view>
    <view>user count: {{userList.length}}</view>
    <view v-for="(item,index) in (userList)" :key="item.userID">{{item.userID}}|mainV:{{item.hasMainVideo||false}}|mainA:{{item.hasMainAudio||false}}|auxV:{{item.hasAuxVideo||false}}</view>
    <view>stream count: {{streamList.length}}</view>
    <view v-for="(item,index) in (streamList)" :key="item.streamID">{{item.userID}}|{{item.streamType}}|
      SubV:<button :class="(!item.muteVideo?'':'false')+' debug-btn'" @click="_debugToggleRemoteVideo" hover-class="button-hover" :data-user-i-d="item.userID" :data-stream-type="item.streamType">{{!item.muteVideo}}</button>|
      SubA:<button :class="(!item.muteAudio?'':'false')+' debug-btn'" @click="_debugToggleRemoteAudio" hover-class="button-hover" :data-user-i-d="item.userID" :data-stream-type="item.streamType">{{!item.muteAudio}}</button></view>
  </view>
...
global['__wxRoute'] = 'trtc-room/trtc-room'
...
export default global['__wxComponents']['trtc-room/trtc-room']
</script>
